...
In ApolloLink ist es möglich jedem Interface eine oder mehrere so genannte Nodes zugewiesen werden, wobei jede Node eine Steuergerät oder der gleichen darstellt.
Jede Node hat wiederum eine so genannte #acr("DBC") Datei zugewiesen, wodurch es möglich wird die Signale, die gesendet werden zu verstehen, und später zu visualisieren.
...
Nachdem einige Programme gelöscht und neu installiert wurden, ließ sich aber auch die #acr("CLI") problemlos aufsetzen, und hat ab da tadellos mit Node und C++ zusammengespielt.
...
#linebreak()
Kompiliert wird der gesamte Nativ-teil mit dem so genannten Node-GYP Tool. Selbiges würde ebenfalls über #acr("NPM") Installiert und bildet die Schnittelle zum Visual Studio 2022 Compiler. Node-GYP kümmert sich dabei darum das alle Dependencies die die spätere NodeJs runtime benötigt inkludiert sind, und das die #acr("NAPI") header richtig sind. #linebreak()
#acr("NAPI") ist die sogenannte Node-API. Sie wird darüber genutzt damit der Native Teil auch mit späteren Versionen der NodeJs Runtime kompatibel ist.
...
=== NodeGyp bauen
Da es sich bei Node-Native im Zusammenhang mit #acr("NAPI") und NodeGyp um das Betriebssystem spezifischen Teil handelt, muss dieser Teil auch auf dem Ziel OS gebaut werden.
Da es sich bei meiner GitLab Infrastruktur allerdings ausschließlich um Docker Exekutor handelt, erwies sich dies als recht Zeitintensiv. Die Lösung war, den  build Schritte in einem Docker Container mittels der Linux Kompatibilitätsschicht "WINE" auszulagern. dadurch wird ein Windows Umgebung vorgegaukelt, obwohl eigentlich ein UNIX System verwendet wird.
...
Bei Electron handelt es sich um eine Laufzeitumgebung die es mittels der NodeJs V8 Umgebung erlaubt Web Code (also #acr("JS") und HTML) nativ auf Windows Mac oder Linux laufen zu lassen. Zudem erlaubt dieses Framework das auch Node-Native #acr("LIB") verwendet werden können. Normalerweise ist die Verwendung diesen nämlich nur auf Vanilla Node beschränkt. Um sicherzustellen das die Native libraries auch bei upgrade der Electron Version problemlos verwendet werden können, wurde auf die #acr("NAPI")-Schnittstelle gesetzt, die durch ihre hohe versionsübergreifende Stabilität überzeugt.

== Schreiben der Dokumentation
Aus Gründen des persönlichen Interesses ist diese gesamte Dokumentation mittels des neuen Setz-Systems namens Typst geschrieben.
Typst erlaubt es ein gesamte Dokument analog zu einer normalen Code Base zu behandeln, Sprich dieses Dokument wurde ebenfalls in einem Code Editor geschrieben, ganz ohne den Einsatz von MS Word oder dergleichen.
